...
OpenFASTLoadProvider::OpenFASTLoadProvider(
    OpenFASTLoadProviderConfig config,
    std::unique_ptr<OpenFASTAdapter> adapter)
    : config_(std::move(config))
    , adapter_(std::move(adapter))
{
    require(static_cast<bool>(adapter_), "OpenFASTLoadProvider requires a non-null adapter");
}
...
void OpenFASTLoadProvider::initialize(const PlatformState& initialState)
{
    close();

    require(config_.openFast.platformReferencePoint.frame == FrameId::global,
            "OpenFASTLoadProvider requires a global-frame platform reference point");
    require(config_.openFast.turbineInterfacePoint.frame == FrameId::global,
            "OpenFASTLoadProvider requires a global-frame turbine interface point");

    adapter_->initialize(config_.openFast, initialState);
    openDebugLog();
    initialized_ = true;
}

Wrench OpenFASTLoadProvider::evaluate(const PlatformState& platformState, double dt)
{
    require(initialized_, "OpenFASTLoadProvider evaluate() called before initialize()");

    const OpenFASTResult result = adapter_->step(platformState, dt);
...
void OpenFASTLoadProvider::close()
{
    if (adapter_) {
        adapter_->close();
    }
...
void OpenFASTLoadProvider::openDebugLog()
{
    ...
    if (!debugStream_.is_open()) {
        throw std::runtime_error(
            "Failed to open OpenFAST debug log at '" + config_.debugCsvPath + "'");
    }

    writeDebugHeader();
}

void OpenFASTLoadProvider::writeDebugHeader()
{
    if (!debugStream_.is_open()) {
        return;
    }
    ...
    debugStream_
        ...
        << "rotor_speed_rad_s,generator_power_w,"
        << "tower_top_dx,tower_top_dy,tower_top_dz,"
        << "controller_ok,status_code,status_message\n";
}

void OpenFASTLoadProvider::writeDebugRow(
    ...
    const Wrench& appliedLoad)
{
    ...
    debugStream_
        ...
        << appliedPoint.y << ','
        << appliedPoint.z << ','
        << result.rotorSpeedRadPerSec << ','
        << result.generatorPowerW << ','
        << result.towerTopDisplacementGlobal.x << ','
        << result.towerTopDisplacementGlobal.y << ','
        << result.towerTopDisplacementGlobal.z << ','
        << (result.controllerOk ? 1 : 0) << ','
        << result.statusCode << ','
